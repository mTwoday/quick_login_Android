...
2. 由于运营商取号能力是通过数据网关实现，首次取号过程必须在数据流量打开的情况下才能进行（WiFi和数据流量同时打开时，SDK会强制切换到数据流量执行取号逻辑，将会消耗用户少量流量），当信号弱或者网络有干扰时，时延会高于平均值，取号成功率较低。
3. 本SDK同时提供一键登录、本机号码校验、应用功能，开发者根据实际的需求调用对应方法和接口。
4. 对于双卡手机，移动认证SDK只对当前的流量卡取号。
...
1. ```java
   dependencies {
       ……
       implementation files('libs/quicklogin_sso_android_10.0.2.jar')
       ……
   }
   ```
...
所有的SDK登录相关接口调用，都会传入一个回调，用于接收SDK返回的调用结果。结果以`JsonObject`的形式传递，`TokenListener`的实现示例代码如下：
...
SDK单点登录时的界面由SDK实现，为了能让开发者修改布局，故把布局文件，资源文件提供给开发者自行修改，需要开发者把demo中SDK所需的布局文件（demo中res-umc文件夹下的文件）放到与res同级；同时，在app的gradle文件中加入：
    sourceSets {
        main {
...
在接入应用连接功能之前，开发者必须先按照1.1接入流程，在中国移动开发者社区注册开发者账号，创建包含移动认证能力的应用A和应用B，获取相应的AppId和AppKey，并且在开发者社区中勾选一键登录能力，配置应用服务器出口ip地址。

邮件通知移动方（13802883476@139.com）配置A（跳出方）和B（跳入方）单点登录关系，包括A方APPID及APPkey，B方APPID及APPkey，及授权页面以下9个配置项；

| 元素       | 配置内容            | 配置内容含义                 | 默认值       |
| ---------- | ------------------- | ---------------------------- | ------------ |
| 导航栏     | navBgColor          | 导航栏背景颜色               | #0378C9      |
|            | navAttrTitle        | 导航栏应用名称               | 统一认证     |
|            | NavAttrTitleColr    | 导航栏应用名称颜色           | #FFFFFF      |
|            | NavLeftImgColr      | 导航栏返回图标颜色           | #FFFFFF      |
| 应用logo   | APPLogo             | 应用图片                     | 中国移动logo |
| 本机号码栏 | 不支持修改          |                              |              |
| 登录按钮   | logAbleButtonBgColr | 登录按钮背景颜色             | #0378C9      |
|            | logButtonTextColr   | 登录按钮文字颜色             | #FFFFFF      |
|            | logButtonShape      | 登录按钮圆角弧度             | 0            |
|            | UserServiceTextColr | 《中国移动认证服务协议》颜色 | #0378C9      |
| 版权声明   | 不支持修改          |                              |              |
|            |                     |                              |              |

4.2. 流程说明 

移动认证一键登录允许开发者在用户同意授权后，在客户端侧获取`接口调用凭证`（token），第三方B应用服务器携带token调用`获取手机号码接口`，实现获取当前授权登录用户的手机号码等信息。

整体流程为：

1. 开发者调用取号方法，取号成功后，SDK将缓存取号临时凭证scrip；
2. 开发者调用单点登录授权方法；
3. 在单点登录授权页用户同意应用获取本机号码，调用授权登录方法，获取B应用的接口调用凭证token；
4. 携带token进行接口调用，获取用户的手机号码信息。
...
| 参数     | 类型          | 说明                                                         |
| :------- | :------------ | :----------------------------------------------------------- |
| appId    | String        | A应用的AppID，在开发者社区创建应用时获取                     |
| appkey   | String        | A应用密钥，在开发者社区创建应用时获取                        |
| listener | TokenListener | TokenListener为回调监听器，是一个java接口，需要调用者自己实现；TokenListener是接口中的认证登录token回调接口，OnGetTokenComplete是该接口中唯一的抽象方法，即void OnGetTokenComplete(JSONObject  jsonobj) |
...
确保临时取号凭证scrip有效，开发者可以调用B应用授权方法，获取B应用的接口凭证token。为了保证scrip的有效性，建议开发者在调用B应用授权登录方法前，再次调用取号方法。（如果scrip有效，再次调用取号方法的耗时非常少）
...
//4.用户点击授权按钮时，调用loginAuth方法，传入应用定义的activity，获取token
    mAuthnHelper.umcLoginOtherApp(Constant.APP_ID, Constant.APP_KEY, Constant.OTHER_APP_ID, mListener);
    ……
```
...
```java
public void umcLoginOtherApp(final String appId, 
                      final String appKey, 
                      final String authenticatedAppid,
                      final TokenListener listener)
```
...
| 参数               | 类型          | 说明                                                         |
| :----------------- | :------------ | :----------------------------------------------------------- |
| appId              | String        | A应用的AppID                                                 |
| appkey             | String        | A应用密钥                                                    |
| authenticatedAppid | String        | 需要单点的B应用AppID                                         |
| listener           | TokenListener | TokenListener为回调监听器，是一个java接口，需要调用者自己实现；TokenListener是接口中的认证登录token回调接口，OnGetTokenComplete是该接口中唯一的抽象方法，即void OnGetTokenComplete(JSONObject  jsonobj) |
...
| ----------- | ------ | ------------------------------------------------------------ |
| resultCode  | String | 接口返回码，“103000”为成功                                   |
| authType    | String | 登录方式：1：WIFI下网关鉴权</br>2：网关鉴权</br>0：其他      |
| authTypeDes | String | 登录方式描述                                                 |
| token       | String | 成功时返回：B应用临时凭证，token有效期2min，一次有效；同一用户（手机号）10分钟内获取token且未使用的数量不超过30个 |
| openId      | String | 成功时返回：B应用用户身份唯一标识。（activity传入null时，不返回） |

## 4.5. 获取手机号码

开发者获取到B应用的token后，需要将token传递到应用服务器，由B应用服务器发起获取用户手机号接口的调用。
...
| systemtime          | 必选 | 请求消息发送的系统时间，精确到毫秒，共17位，格式：20121227180001165 |
| strictcheck         | 必选 | 暂时填写"0"                                                  |
| appid               | 必选 | 业务在统一认证申请的B应用id                                  |
| expandparams        | 可选 | 扩展参数                                                     |
| token               | 必选 | 需要解析的凭证值。                                           |
| sign                | 必选 | 当**encryptionalgorithm≠"RSA"**时，sign = MD5（appid + version + msgid + systemtime + strictcheck + token + appkey)（注：“+”号为合并意思，不包含在被加密的字符串中，appkey为B应用的appkey），输出32位大写字母；</br>当**encryptionalgorithm="RSA"**，业务端RSA私钥签名（appid+token）, 服务端使用业务端提供的公钥验证签名（公钥可以在开发者社区配置）。 |
| encryptionalgorithm | 可选 | 推荐使用。开发者如果需要使用非对称加密算法时，填写“RSA”。（当该值不设置为“RSA”时，执行MD5签名校验） |
...
业务方A应用调用此方法，能获取到供业务方B应用使用的取号凭证token。
...
| appId              | String        | A应用的AppID                                                 |
| appkey             | String        | A应用密钥                                                    |
| authenticatedAppid | String        | B应用的AppID                                                 |
| listener           | TokenListener | TokenListener为回调监听器，是一个java接口，需要调用者自己实现；TokenListener是接口中的认证登录token回调接口，OnGetTokenComplete是该接口中唯一的抽象方法，即void OnGetTokenComplete(JSONObject  jsonobj) |
...
| authType    | String | 登录方式：1：WIFI下网关鉴权</br>2：网关鉴权</br>0：其他      |
| authTypeDes | String | 登录方式描述                                                 |
| token       | String | 成功时返回：B应用的临时凭证，token有效期2min，一次有效；同一用户（手机号）10分钟内获取token且未使用的数量不超过30个 |
| openId      | String | 成功时返回：B应用的用户身份唯一标识。（activity传入null时，不返回） |
