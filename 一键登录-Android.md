...
1、根据《开发者接入流程文档》，在开发者社区（dev.10086.cn)申请并创建应用，获取appid、appkey，申请相关能力，填写应用相关资料等；
...
创建一个授权页面`activity`，按照授权页面规范设计布局。

**第四步：取号**

调用取号方法`getPhoneInfo`，获取手机号码掩码（139xxxx0000）。

**第五步：授权**

第四步成功后，调用授权登录方法`loginAuth`，将第三步中创建的授权页`activity`作为参数传入`loginAuth`方法中，成功后获取`token`和`openId`

**第六步：获取用户手机号码**

客户端将SDK返回的token上传到应用服务器，应用服务器调用`获取用户信息`接口，获取用户手机号码。
...
`AuthnHelper`是SDK的功能入口，所有的接口调用都得通过`AuthnHelper`进行调用。因此，调用SDK，首先需要创建一个`AuthnHelper`实例，示例代码参考1.5.1小节第一步

**第二步：实现回调**

所有的SDK接口调用，都会传入一个回调，用以接收SDK返回的调用结果。结果以`JsonObject`的形式传递，`TokenListener`的实现示例代码参考1.5.1小节第二步
...
第三步成功后，调用授权登录方法`loginAuth`，`activity`参数传`null`值，成功将返回`token`。

**第五步：号码校验**

客户端将SDK返回的`token`和用户输入的待校验手机号码上传到应用服务器，应用服务器调用`本机号码校验`接口，获取用户手机号码。
...
1、页面必须包含登录/注册按钮，授权登录方法`loginAuth`必须通过触发该按钮时调用；

2、登录按钮文字描述必须包含**“登录”**或**“注册”**等文字，不得诱导用户授权；

3、需要用户明确知悉使用本机号码作为登录账号。如果应用通过取号方法`getPhoneInfo`获得手机号码掩码，并且将掩码显示在授权页面显著的位置上，授权页面可以不显示**“本机号码”**等文字，否则，应用必须在页面显著位置告知用户正在使用**“本机号码”**作为账号登录或注册；
